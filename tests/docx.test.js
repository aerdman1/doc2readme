...
/* The trapdoor this guards: "Contents" is often the largest heading in a
 * document — in a PDF the level comes from font size, and a contents page is
 * usually set larger than the chapters it lists. Ending the dropped section at
 * "the next heading of the same or higher level" then never terminates, and the
 * whole document goes with it. This silently emptied six of eight real guides
 * down to their cover blurb. */
test('a Contents heading above every other level does not eat the document', async () => {
  const { body } = await convert(
    heading(1, 'Contents') + para('Chapter 1: Intro .......... 5') +
    heading(2, 'Chapter 1: Intro') + para('keep this body') +
    heading(2, 'Chapter 2: More') + para('and keep this too'));
  assert.doesNotMatch(body, /\.{6,}/, 'the contents listing survived');
  assert.match(body, /keep this body/, 'the document after Contents was dropped');
  assert.match(body, /and keep this too/);
  assert.match(body, /^## Chapter 1: Intro$/m);
  assert.match(body, /^## Chapter 2: More$/m);
});

test('a dropped section that really is last still drops its body', async () => {
  const { body } = await convert(
    heading(1, 'Intro') + para('keep me') +
    heading(1, 'Revision History') + para('1.0 first draft'));
  assert.match(body, /keep me/);
